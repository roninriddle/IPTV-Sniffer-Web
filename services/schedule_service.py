...
ScheduleTask = Callable[[dict[str, Any]], dict[str, Any]]


class ScheduleService:
    def __init__(self, logger: AppLogger, settings_store: SettingsStore, task_runner: ScheduleTask) -> None:
        self.logger = logger
    ...
    def _loop(self) -> None:
        while not self._stop_event.wait(5):
            with self._lock:
                ...
                due = enabled and not running and isinstance(next_run_at, (int, float)) and time.time() >= float(next_run_at)
            if due:
                self._run_due()
    ...
    def _run_due(self) -> None:
        ...
        if not schedule.get("schedule_enabled"):
            with self._lock:
                ...
                self._state["last_message"] = "定时 EPG 未启用"
            return
        self._execute(settings, schedule, manual=False)

    def _execute(self, settings: dict[str, Any], schedule: dict[str, Any], manual: bool) -> dict[str, Any]:
        with self._lock:
            if self._state.get("running"):
                raise RuntimeError("定时 EPG 正在执行，请稍后再试")
        ...
        try:
            ...
            with self._lock:
                ...
                if manual and not schedule.get("schedule_enabled"):
                    self._state["next_run_at"] = None
        return self.status()
